...
| H-003 | The displayed escalation level is lower than the clinically indicated level | Trigger data is absent or incorrectly mapped, or raw carer concern, clinical intuition, specific concern, AVPU change, temperature, or sepsis data is not converted to an explicit trigger | Delayed senior review, treatment, or emergency response | Explicit `CI`, `CQ`, and `SC` trigger levels participate in highest-level selection with provenance; automatic trigger derivation remains open under R10; host controls `TC-004` and `TC-005` | Component maintainer and integrator | Open | [`spec/escalation.md`](spec/escalation.md), [`chart/npews-scorer.js`](chart/npews-scorer.js), roadmap R10 |
...
| SC-009 | Apply every supported non-score trigger, retain provenance, and choose the highest indicated escalation | Partially implemented for explicit `CI`, `CQ`, and `SC` levels | H-003 | [`chart/npews-scorer.js`](chart/npews-scorer.js), scorer tests, roadmap R10, and [`spec/escalation.md`](spec/escalation.md) |
