...
- Explicit host-supplied Clinical Intuition, Carer Question, and Specific Concern levels participate in escalation, but automatic derivation from raw responses, AVPU, temperature, and sepsis is incomplete.
...
| H-003 | The displayed escalation level is lower than the clinically indicated level | Trigger data is absent or incorrectly mapped; raw concern has no selected level; AVPU state is interpreted incorrectly; or specific-concern/sepsis data is omitted | Delayed senior review, treatment, or emergency response | Explicit and derived `CI`, `CQ`, and `SC` decisions participate in highest-level selection with criterion/origin provenance; unresolved raw concerns and abnormal-temperature sepsis prompts remain visible; automatic rules have unit tests and an isolated demo scenario; clinician review confirmed AVPU mapping against the NHS England reference chart on 2026-08-11; host controls `TC-004` and `TC-005` | Component maintainer and integrator | Open | [`spec/escalation.md`](spec/escalation.md), [`spec/decisions.md`](spec/decisions.md), [`chart/npews-scorer.js`](chart/npews-scorer.js), [`test/scoring/non-score-escalation.test.js`](test/scoring/non-score-escalation.test.js), roadmap R10 |
| H-004 | The chart visually implies a false trend or misrepresents a value | Line drawn over a skipped observation, line joining incompatible oxygen modalities, inaccurate Canvas geometry, overlapping UI, incorrect colour, or responsive-layout defect | Clinician misinterprets deterioration, recovery, or severity | Skip and modality line breaks in renderer; exact plotted positions; documented interim screen tokens pending authoritative NHSE RGB values under issue #5 and R50; manual visual checks; browser and visual regression controls planned under R15-R19 | Component maintainer | Open | [`chart/chart.js`](chart/chart.js), [`spec/implementation-notes.md`](spec/implementation-notes.md), [`spec/visual-regression-testing-plan.md`](spec/visual-regression-testing-plan.md) |
| H-005 | The chart presents stale or internally inconsistent patient state | Time-window selection affects latest status, data reassignment or reconnect retains stale state, or multiple components share global DOM/state | Clinician acts on old observations or mismatched score, chart, and patient identity | Latest-score invariant implemented; historical playback excluded from the component; the demo uses a labelled truncated dataset; delegated time-window controls survive shell replacement; one-instance warning; lifecycle and browser work planned under R15 and R25-R27; `TC-007` | Component maintainer and integrator | Open | [`spec/implementation-notes.md`](spec/implementation-notes.md), [`spec/web-component-phase2-spec.md`](spec/web-component-phase2-spec.md) |
| H-006 | Missing, malformed, or unsupported data appears normal or is silently omitted | Null/invalid values, unknown category/device code, missing required fields, unsupported unit, unavailable blood pressure, or host mapping failure | False reassurance, incomplete score, hidden deterioration, or inability to interpret the chart | Defined data model; skip-reason support; FHIR and scorer tests; malformed-input browser tests planned under R15; missing-BP policy research under R53; `TC-001`; explicit validation/failure contract still required | Component maintainer and integrator | Open | [`spec/data-model.md`](spec/data-model.md), [`spec/fhir.md`](spec/fhir.md), roadmap R15 and R53 |
...
| SC-006 | Keep latest PEWS and escalation presentation independent of the selected time window | Implemented, browser evidence incomplete | H-005 | [`spec/implementation-notes.md`](spec/implementation-notes.md), [`chart/chart.js`](chart/chart.js) |
...
| SC-009 | Apply every supported non-score trigger, retain provenance, surface unresolved concern/warning states, and choose the highest indicated escalation | Implemented, clinician mapping review recorded; formal safety assurance pending | H-003 | [`chart/npews-scorer.js`](chart/npews-scorer.js), [`test/scoring/non-score-escalation.test.js`](test/scoring/non-score-escalation.test.js), roadmap R10, and [`spec/escalation.md`](spec/escalation.md) |
...
| SC-012 | Isolate component state and lifecycle so reassignment, reconnection, and multiple instances cannot cross-contaminate | Planned | H-005 | Roadmap R25-R27 |
| SC-013 | Keep historical playback outside the component and label the demo's simulated current observation explicitly | Implemented, browser evidence incomplete | H-005 | [`demo/demo.js`](demo/demo.js), [`spec/implementation-notes.md`](spec/implementation-notes.md) |
...
- Complete controls planned under R11-R19 and R25-R27 where required for the intended release.
...
| --- | --- | --- | --- | --- |
| 2026-08-11 | Clinician maintainer | R10 non-score escalation, including AVPU current-value interpretation | AVPU V/P/U mapping confirmed against the NHS England reference chart; R10 implementation accepted for this development stage | Formal safety assurance and integrated workflow validation remain required before clinical deployment |
