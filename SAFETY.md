...
| H-003 | The displayed escalation level is lower than the clinically indicated level | Trigger data is absent or incorrectly mapped; raw concern has no selected level; AVPU “change” is interpreted incorrectly; or specific-concern/sepsis data is omitted | Delayed senior review, treatment, or emergency response | Explicit and derived `CI`, `CQ`, and `SC` decisions participate in highest-level selection with criterion/origin provenance; unresolved raw concerns and abnormal-temperature sepsis prompts remain visible; automatic rules have unit tests and an isolated demo scenario; AVPU interpretation and control effectiveness require clinical review under R10; host controls `TC-004` and `TC-005` | Component maintainer and integrator | Open | [`spec/escalation.md`](spec/escalation.md), [`spec/decisions.md`](spec/decisions.md), [`chart/npews-scorer.js`](chart/npews-scorer.js), [`test/scoring/non-score-escalation.test.js`](test/scoring/non-score-escalation.test.js), roadmap R10 |
...
| SC-009 | Apply every supported non-score trigger, retain provenance, surface unresolved concern/warning states, and choose the highest indicated escalation | Implemented, clinical review pending | H-003 | [`chart/npews-scorer.js`](chart/npews-scorer.js), [`test/scoring/non-score-escalation.test.js`](test/scoring/non-score-escalation.test.js), roadmap R10, and [`spec/escalation.md`](spec/escalation.md) |
