...
- [x] **R46 - Define clinical policy at age-band boundaries.** The deterministic switch and explicit divider at the 1st, 5th, and 13th birthdays are preserved without the `C2.2` alternate-form override. The open research question, current behaviour, hazard implications, senior-discretion context, and governance route are documented in [`age-band-boundary-policy.md`](./age-band-boundary-policy.md).
...
- [x] **R23 - Define typed presentation options.** The separate assignable `.options` object now exposes `showDemographics` (boolean, default `true`), `showValues` (boolean, default `true`), `initialLayout` (`'landscape' | 'portrait' | 'mobile' | 'auto'`, default `'auto'`), and `initialTimeWindow` (positive number in hours, default `24`). A JSDoc `@typedef NpewsChartOptions` documents the contract. The demo harness provides controls for all four. Options reassignment triggers a re-render and preserves the setting across data changes.
...
- [x] **R12 - Add general CI.** `.github/workflows/ci.yml` runs `npm ci`, `npm test`, and `npm run generate:scoring:check` on every push to `main` and on pull requests, plus REUSE licence compliance via `fsfe/reuse-action` and GitHub Actions security scanning via `zizmor`. Action SHAs are pinned with `# vX.Y.Z` comments per house style. Dependabot now also tracks `github-actions` updates.
