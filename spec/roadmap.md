...
- [x] **R50 - Verify exact reference-chart colours.** Direct PDF samples, confirmed across all four supplied reference images, establish white `#ffffff`, yellow `#fffa99`, orange `#fbc07b`, and pink `#fabfda` as the source sRGB palette. The previous approximations have been corrected and the sampling evidence and change-governance rule are recorded in [`implementation-notes.md`](./implementation-notes.md).
