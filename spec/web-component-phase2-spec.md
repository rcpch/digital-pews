...
The completed Phase 1 element renders into light DOM, mounts a shell containing fixed ids, and delegates to a renderer with module-global patient, observation, segment, and view state. It intentionally supports one chart per document. Reassigning `.data` rebuilds the shell; delegated time-window handling survives that replacement. Per-instance state, complete connection/disconnection cleanup, and multiple simultaneous charts remain Phase 2 responsibilities.

Browser correctness work in roadmap R14-R19 must land before Web Component Phase 2 hardening begins. Shadow DOM must not hide existing lifecycle defects or make visual comparison harder.
